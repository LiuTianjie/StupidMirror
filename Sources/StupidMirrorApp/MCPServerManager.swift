...
final class MCPServerManager: ObservableObject {
    ...
    Automate iPhones through StupidMirror. Start with list_devices. With multiple devices always pass device_id. Unactivated installations can mirror one device at a time; control tools require activation. Start the mirror for a live frame and connect control for native accessibility and actions. For routine navigation use tap_text with all candidate labels in one call; use find_any_element when inspection is needed before tapping. With include_ocr=true these tools inspect the live mirror with local Apple Vision first, then fetch at most one WDA hierarchy when pixels do not expose a label. Never probe synonyms with repeated find_element calls. Use observe_screen with include_image=true when neither OCR nor accessibility describes an icon, canvas, or custom control; inspect the screenshot and use a targeted normalized coordinate. Request include_accessibility=true or get_ui_tree only for explicit hierarchy debugging because WDA /source can take seconds and may disturb a focused text field. Once a field is focused, use replace_text to overwrite it or clear_text to empty it without an intermediate accessibility query; use type_text only to append. For user-guided operation, use highlight_clickable_elements to number every clickable target or highlight_elements for selected observed targets; these overlays never tap or change the iPhone. Prefer fast OCR unless small or ambiguous text needs accurate mode. AI actions are visibly highlighted on the Mac mirror. Perform one targeted observation after actions that need verification.
    """
}
