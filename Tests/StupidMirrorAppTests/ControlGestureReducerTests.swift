...
final class ControlGestureReducerTests: XCTestCase {
    ...
    func testInstalledWDAFallbackDoesNotHideActionableUserErrors() {
        ...
        XCTAssertTrue(AppiumError.shouldFallbackToWDAInstall(afterInstalledWDAError: AppiumError.httpStatus(500, #"{"value":{"message":"Unable to launch WebDriverAgent. Original error: Failed to start the preinstalled WebDriverAgent"}}"#)))
        XCTAssertFalse(AppiumError.shouldFallbackToWDAInstall(afterInstalledWDAError: AppiumError.httpStatus(500, #"{"value":{"message":"connect ECONNREFUSED 127.0.0.1:8100"}}"#)))
